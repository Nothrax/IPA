...
int gen_gl_list_from(const char *file, int listId)
{
    ...
    */
    ...
    if (file!=NULL)//-->ouverture OK
    {
        ...
        pts = (Point*) malloc(sizeof(Point)*nbPts);
        texcoords = (TexCoor*) malloc(sizeof(TexCoor)*nbTexCoors);
    ...
    GLuint texture = loadTexture(mtlPngFile,true);
    printf(mtlPngFile);
    printf("%d", texture);
